...
export default function McyStickyCta() {
  ...
  return (
    <div
      aria-hidden={!visible}
      className={`fixed inset-x-0 bottom-0 z-50 border-t border-lux-border bg-white transition-transform duration-200 motion-reduce:transition-none ${visible ? "translate-y-0" : "pointer-events-none translate-y-full"}`}
    >
      <div className="mx-auto grid max-w-[520px] grid-cols-2 gap-3 px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-3">
        <a
          href={PHONE_TEL}
          tabIndex={visible ? undefined : -1}
          data-gtm="cta_tel_sticky"
          className={`relative flex min-h-[62px] overflow-hidden flex-col items-center justify-center rounded-[12px] mcy-contact-phone px-2 py-2 text-center text-white ${phoneClosed ? "order-2" : "order-1"}`}
        >
          <McyCtaShine />
          <span className="relative text-[14px] font-bold">電話で相談する</span>
          {/* nowrap segments so a 320px column breaks at the separator, never mid-word */}
          <span className="relative mt-1 text-[12px] leading-4">
            {phoneClosed ? (
              <>
                <span className="whitespace-nowrap">受付時間外・</span>
                <span className="whitespace-nowrap">朝9時から</span>
              </>
            ) : (
              "9:00〜21:00"
            )}
        ...
        <a
          ...
          className={`relative flex min-h-[62px] overflow-hidden flex-col items-center justify-center rounded-[12px] border border-[#27804d] mcy-contact-line px-2 py-2 text-center text-white ${phoneClosed ? "order-1" : "order-2"}`}
        >
          <McyCtaShine />
          <span className="relative text-[14px] font-bold">LINEで相談する</span>
          <span className="relative mt-1 text-[12px] leading-4">
            <span className="whitespace-nowrap">写真なしOK・</span>
            <span className="whitespace-nowrap">24時間受付</span>
          </span>
        </a>
      </div>
    </div>
